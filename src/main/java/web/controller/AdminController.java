...
public class AdminController {
	...
	public String admin(ModelMap model) {
		try {
			...
			if (a==0) {
				...
				model.addAttribute("isStep1", 1);
				model.addAttribute("isStep2", 1);
	...
	public int trainData() {
		...
		try {
			...
			Process p = rt.exec("D:/HocTap/Python/Python310/python.exe "
								+PREMAXTRIXPY_STRING
	...
	public int getRatingUser() {
		...
		try {
			Runtime rt = Runtime.getRuntime();
			Process p = rt.exec("D:/HocTap/Python/Python310/python.exe "+GETRATING_STRING
								+" "+fileUserPred+" "+fileData);
	...
	public int getSongSim() {
		try {
			Runtime rt = Runtime.getRuntime();
			Process p = rt.exec("D:/HocTap/Python/Python310/python.exe "+GETSONGSIM_STRING
								+" "+fileSongSim+" "+fileData);
			BufferedReader r = new BufferedReader(new InputStreamReader(p.getInputStream()));
			String line;
